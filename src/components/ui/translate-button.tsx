...
export function TranslateButton({ ...buttonProps }: TranslateButtonProps) {
    const { language, setLanguage, t } = useTranslation();

    return (
        <DropdownMenu>
            <DropdownMenuTrigger asChild>
                <Button {...buttonProps} type="button" variant="link" size="icon" aria-label={t('changeLanguage')} title={t('changeLanguage')}>
            ...
            <DropdownMenuContent align="end" className="w-44">
                ...
                <DropdownMenuItem onSelect={() => setLanguage('en')}>
                    ...
                    <span className="text-xs text-zinc-500">{language === 'en' ? '✓ EN' : 'EN'}</span>
                </DropdownMenuItem>

                <DropdownMenuItem onSelect={() => setLanguage('cs')}>
                    <span className="flex-1">{t('czech')}</span>
                    <span className="text-xs text-zinc-500">{language === 'cs' ? '✓ CS' : 'CS'}</span>
                </DropdownMenuItem>
            </DropdownMenuContent>
        </DropdownMenu>
    );
